  ...
  <ul class="menu-content">
      @if (isset($menu))
        @foreach ($menu as $submenu)
              ...
              @if(!checkAccess($submenu->name,'show_module'))
                  @continue
              @endif


          <li {{ $submenu->slug === Route::currentRouteName() ? 'class=active' : '' }}>
            <a href="@isset($submenu->url) @if($submenu->url !=''){{route($submenu->url)}}@endif @endisset"  class="d-flex align-items-center" @if(isset($submenu->newTab)){{"target=_blank"}}@endif>
            <span class="menu-item text-truncate text-uppercase">{{ __($submenu->name)}}</span>
            </a>
  ...
  <ul class="dropdown-menu">
      @if(isset($menu))
        @foreach ($menu as $submenu)
          ...
          <a class="dropdown-item align-items-center d-flex align-items-center @if(isset($submenu->submenu)){{'dropdown-toggle'}}@endif" href="{{route($submenu->url)}}" data-toggle="dropdown" @if(isset($submenu->newTab)){{"target=_blank"}}@endif>
