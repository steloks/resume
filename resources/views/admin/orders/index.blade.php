...
<div class="row">
    <div class="col-12 mt-1">
        ...
        <div class="row">
            <div class="filters col-12 col-md-4 mb-1">
                <label for="order_status_id">{{__('Order status')}}</label>
                <select id="order_status_id" class="filter form-control" name="order_status_id">
            ...
            <div class="filters col-12 col-md-4 mb-1">
                ...
                <select id="area_id" class="filter form-control" name="area_id">
                    <option value="">{{ __('All') }}</option>
            ...
            <div class="filters col-12 col-md-4 mb-1">
                <label for="sub_area_id">{{__('Sub area')}}</label>
                <select id="sub_area_id" class="filter form-control" name="sub_area_id">
                    <option value="">{{ __('All') }}</option>
                    @foreach($subAreas as $subAreaId => $subAreaName)
                        <option value="{{ $subAreaId }}">{{ $subAreaName }}</option>
                    @endforeach
                </select>
            </div>
            <div class="filters col-12 col-md-4 mb-1">
                <label for="object_id">{{__('Object')}}</label>
                <select id="object_id" class="filter form-control" name="object_id">
                    <option value="">{{ __('All') }}</option>
            ...
            <div class="filters col-12 col-md-3 mb-1">
                <label for="order_start_date">{{__('Order created at') . __('(Start)')}}</label>
                <input type="text" class="form-control js-order-date" name="order_start_date" id="order_start_date">
            </div>
            <div class="filters col-12 col-md-3 mb-1">
                <label for="order_end_date">{{ __('(End)') }}</label>
                <input type="text" class="form-control js-order-date" name="order_end_date" id="order_end_date">
            </div>
            <div class="col-12 col-md-2 mb-1 d-flex m-auto" style="margin-top: 23px !important;">
                <a href="javascript:void(0)" class="btn btn-dark text-uppercase js-order-date-reset" title="clear" data-clear>
                    {{ __('Clear') }}
                </a>
            </div>
        ...
        <div class="dataTables_scroll mt-1">
            <table class="table w-100 js-datatable-ajax" id="orders"
                   data-datatable_request_url="{{ route('admin.orders.gridDataAdminOrders') }}"
            >
                <thead>
                <tr>
                    <th></th>
...
@section('page-scripts')
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('.flatpickr-wrapper').addClass('d-flex')
            $('.js-order-date').addClass('filter')

            $(document).on('click', '.js-order-date-reset' , function () {
                $('.filters').find('.js-order-date').each(function () {
                    flatpickr($(this)).clear();
                })
            })
                ...
                $.each(tables, function () {
                    ...
                    let table = $('#' + table_id).DataTable({
                        ...
                        columnDefs: [
                            ...
                            // {responsivePriority: 1, targets: [0, 1, 2, -1], visible: true},
