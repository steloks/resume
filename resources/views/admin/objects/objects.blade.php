    ...
    <div class="row">
        ...
        @if(checkAccess('Objects','create_edit'))
            <div class="col-12 col-md-4 mt-1 dtr-mtc">
                <a href="{{route('admin.objects.add')}}">
                    <button type="button" class="btn btn-dark text-uppercase">{{ __('Add') }}</button>
                </a>
            </div>
        @endif
    </div>
        ...
        <div class="col-12 mt-1">
            <div class="dataTables_scroll">
                <div class="filters">
                    ...
                    <select id="cityId" data-route="{{route('admin.zoning.city.regions')}}" class="filter form-control"
                            name="cityId">
                    ...
                    <select id="area" class="filter form-control" name="areaId">
                        <option class="removeOption" value="0">{{ __('Select City') }}</option>
                    </select>
                </div>
                <table class="table w-100 js-datatable-ajax" id="objects"
                       data-datatable_request_url="{{ route('admin.objects.gridDataObjects') }}"
                >
                    <thead>
                    <tr>
                        <th>{{ __('Id') }}</th>
                        <th>{{ __('Name') }}</th>
                        <th>{{ __('Menus for today') }}</th>
                        <th>{{ __('City') }}</th>
                        <th>{{ __('Area') }}</th>
                        <th>{{ __('Sub areas') }}</th>
                        <th>{{ __('Created by') }}</th>
                ...
                $.each(tables, function () {
                    ...
                    let table = $('#' + table_id).DataTable({
                        ...
                        columnDefs: [
                            ...
                            {responsivePriority: 1, targets: [0, 1, 3, 4], visible: true},
        ...
        $(document).ready(function () {
            $(document).on('change', '#cityId', function () {
                $.ajax({
                    ...
                    success: function (response) {
                        $('.removeOption').remove();
                        $('#area').append('<option class="removeOption" value="">All</option>')
                        $.each(response.areas, function (key, value) {
                            $('#area').append('<option class="removeOption" value="' + value.id + '">' + value.name + '</option>')
                        });
                    },
                    error: function (data) {
                    },
