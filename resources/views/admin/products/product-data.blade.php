...
@section('content')
    <div class="row">
        ...
        <div class="col-12 col-md-6 mt-1 dtr-mtc">
            <button type="button" class="btn btn-wine text-uppercase excelButton">Export - excel</button>
        </div>
    </div>
    <div class="row">
        <div class="col-12 mt-1">
            <form class="submitExcel" method="POST" action="{{route('admin.products.product.data.excel')}}">
                @csrf
                <div class="filters">
                    ...
                    <input type="text" class="filter form-control" name="product">
                    <label>{{__('Wasted')}}</label>
                    <input type="checkbox" class="filter form-control" name="wasted" value="1">
                    <label>{{__('Use Date')}}</label>
                    <input class="filter form-control" id="batchUseDate" type="text" name="batchUseDate">
                    <label>{{__('Purchase Date')}}</label>
                    <input class="filter form-control" id="batchPurchaseDate" type="text" name="batchPurchaseDate">
                </div>
            </form>
            <div class="dataTables_scroll">
                <table class="table w-100 js-datatable-ajax" id="categories"
                       data-datatable_request_url="{{ route('admin.products.product.data.grid') }}"
                >
                    <thead>
                    <tr>
                        <th>{{ __('Batch') }} №</th>
                        ...
                        <th>{{ __('Date') }}</th>
                        <th>{{ __('Left Amount') }}</th>
                        <th>{{ __('Delivery Price') }}</th>
                        <th>{{ __('Sell Price') }}</th>
                        <th>{{ __('Expire Date') }}</th>
                        <th>{{ __('Wasted') }}</th>
                        <th>{{ __('Wasted Date') }}</th>
                        <th>{{ __('Object') }}</th>
                    </tr>
                    </thead>
                ...
                $.each(tables, function () {
                    ...
                    let table = $('#' + table_id).DataTable({
                        ...
                        "initComplete": function (settings, json) {
                            // $(table.column(2).footer()).html(json.totalBought)
                            $(table.column(2).footer()).html(json.totalPurchaseQty)
                            $(table.column(4).footer()).html(json.totalPrepQty)
                            $(table.column(6).footer()).html(json.totalResQty)
                            $(table.column(7).footer()).html(json.totalPrice)
                            $(table.column(8).footer()).html(json.totalSellPrice)
                        },

                        columnDefs: [
                            // {responsivePriority: 1, targets: 0, orderable: false},
                            {responsivePriority: 1, targets: [0, 1], visible: true},
