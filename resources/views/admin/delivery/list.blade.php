...
@section('content')
    <div class="row">
        <div class="col-12 col-md-8 mt-1">
            <h4>{{__('Deliveries')}}</h4>
            <p>{{__('Your deliveries for')}}: {{parseDate(getMenusDate())}}@if(!\Illuminate\Support\Facades\Auth::guard('admin')->user()->hasRole('Admin'))/ Courier: {{\Illuminate\Support\Facades\Auth::guard('admin')->user()->name}} @endif</p>
        </div>
        <div class="col-12 col-md-4 mt-1 dtr-mtc">
            <button type="button" class="btn btn-dark text-uppercase"
                    data-bs-target="#dashboard-modal-client-change-status" data-bs-toggle="modal">{{__('Change status')}}
            </button>
        </div>
    </div>
    <div class="row">
        <div class="col-12 mt-1">
            @if(\Illuminate\Support\Facades\Auth::guard('admin')->user()->hasRole('Admin'))
                <div class="filters">
                    ...
                    <select class="filter form-control" name="courierId">
            ...
            <div class="dataTables_scroll">
                ...
                <table class="table w-100 js-datatable-ajax" id="recipes"
                       data-datatable_request_url="{{ route('admin.couriers.deliveries.grid') }}"
                >
                    <thead>
                    <tr>
                        <th></th>
...
@endsection
@include('admin.modals.dashboard-modal-client-change-status');
                ...
                $.each(tables, function () {
                    ...
                    let table = $('#' + table_id).DataTable({
                        ...
                        columnDefs: [
                            ...
                            {responsivePriority: 1, targets: [0, 1, -1, 4], visible: true},
    ...
    <script>
        $(document).ready(function () {
            $(document).on('change', '#selectAll', function (e) {
                if ($(this).is(':checked')) {
                    $('.menu-courier-status-change').prop('checked', true)
                } else {
                    $('.menu-courier-status-change').prop('checked', false)
                }
            })
            $(document).on('click', '#confirm-recipe-status-change', function (e) {
                let dataIds = '';
                $('.menu-courier-status-change:checked').each(function () {
                ...
                $.ajax({
                    dataType: "json",
                    type: "POST",
                    url: $('.menu-status-change').data('route'),
                    data: {
                        'ids': dataIds,
                        'statusId': 8,
                        "_token": "{{ csrf_token() }}"
            ...
            $(document).on('change', '.menu-status-change', function () {
                let status_id = $(this).val();
                console.log($(this).data('route'));
                console.log($(this).parent().parent().find('.menu-courier-status-change').data('ids'))
                $.ajax({
                    dataType: "json",
                    type: "POST",
                    url: $(this).data('route'),
                    data: {
                        "statusId": status_id,
                        "ids": $(this).parent().parent().find('.menu-courier-status-change').data('ids'),
                        "_token": $('[name=csrf-token]').attr('content'),
                    },
                    beforeSend: function () {
                    },
