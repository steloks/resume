...
@section('content')
    <div class="row max-680x-c">
        <div class="col-12 mt-1 mb-2">
            <a href="{{route('admin.kitchen.index')}}" class="brd-a-item mb-1">
                <i class="bx bx-arrow-back"></i> <span>{{__('To the Kitchen')}}</span>
            </a>
            <h4>{{__('Order')}} №{{$order->id}}</h4>
        </div>
    </div>
    <form class="row max-680x-c" method="POST" action="{{route('admin.kitchen.menu.ready')}}">
        @csrf
        ...
        @foreach($order->recipes as $orderRecipe)
            <div class="col-12 mt-1 mb-1">
                <div class="row">
                    <div class="col-12">
                        <p>{{__('Menu')}} №{{$order->id.'-'.$orderRecipe->id}}</p>
                    </div>
                    <div class="col-12 col-md-8 mb-1">
                        <strong>{{$orderRecipe->recipe->name}}</strong>
                    </div>
                    <div class="col-12 col-md-4 form-group mb-1">
                        <fieldset>
                            <div class="checkbox">
                                <input type="checkbox" class="checkbox-input" id="checkbox{{$orderRecipe->id}}"
                                       name="orderRecipeIds[]" value="{{$orderRecipe->id}}">
                                <label for="checkbox{{$orderRecipe->id}}" class="text-uppercase">Приготвено</label>
                            </div>
                            @error('orderRecipeIds')
            ...
            <div class="col-12 mb-3">
                <table class="table w-100 js-datatable-ajax" id="region-menus{{$orderRecipe->id}}"
                       data-datatable_request_url="{{ route('admin.kitchen.cooking.menu.products',['recipeId'=>$orderRecipe->recipe_id,'petId'=>$orderRecipe->pet_id]) }}"
                >
                    <thead>
                    <tr>
                        <th>{{ __('Product') }}</th>
        ...
        @endforeach
        <button type="button" class="btn btn-secondary mark-complete">
    ...
    <script type="text/javascript">
                ...
                $.each(tables, function () {
                    ...
                    let table = $('#' + table_id).DataTable({
                        ...
                        initComplete: function (settings, json) {
                            $(table.column(1).footer()).html(json.totalAmount)
                        },
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.mark-complete', function () {
                let orderRecipeIds = "";
                $("input[name='orderRecipeIds[]']").each(function () {
                    if ($(this).is(':checked')) {
                        orderRecipeIds += $(this).val() + ",";
                    }
                });
                orderRecipeIds = orderRecipeIds.slice(0, -1);
                ...
                $.ajax({
                    dataType: "json",
                    type: "POST",
                    url: "{{route('admin.kitchen.menu.print')}}",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "orderRecipeIds": orderRecipeIds
                    },
                    beforeSend: function () {
                    },
