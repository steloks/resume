...
{{-- page Title --}}
@section('title','Regions - postal code addresses')
{{-- vendor style --}}
@section('page-styles')
    ...
    <div class="row">
        ...
        @if(checkAccess('PC Addresses','create_edit'))
            <div class="col-12 col-md-5 mt-1 dtr-mtc">
                <form enctype="multipart/form-data" class="formImport" action="{{route('admin.zoning.import.csv')}}" method="POST">
                    @csrf
                    <input type="file" id="fileInput" class="d-none" name="csv">
                </form>
                <button type="button" id="clickInput" class="btn btn-light text-uppercase ">Import</button>
                <a href="{{route('admin.zoning.postcode.add')}}">
    ...
    <div class="row">
        <div class="col-12 mt-1">
            <div class="filters">
                ...
                <select id="area" data-route="{{route('admin.zoning.region.subareas')}}" class="filter form-control"
                        name="areaId">
                ...
                <label for="subArea">{{__('Sub Area')}}</label>
                <select id="subArea" class="filter form-control" name="subAreaId">
                    <option class="removeOption" value="">{{ __('Select Area') }}</option>
                </select>
            </div>
            <div class="dataTables_scroll">
                <table class="table w-100 js-datatable-ajax" id="postcodes"
                       data-datatable_request_url="{{ route('admin.zoning.gridDataPostcodes') }}"
                >
                    <thead>
                    <tr>
                        <th></th>
                ...
                $.each(tables, function () {
                    ...
                    let table = $('#' + table_id).DataTable({
                        ...
                        columnDefs: [
                            ...
                            {responsivePriority: 1, targets: [0, 2, 3, -1], visible: true},
        ...
        $(document).ready(function () {
            ...
            $(document).on('change', '#area', function () {
                $.ajax({
                    ...
                    success: function (response) {
                        $('.removeOption').remove();
                        $('#subArea').append('<option class="removeOption" value="">All</option>')
                        $.each(response.subAreas, function (key, value) {
                            $('#subArea').append('<option class="removeOption" value="' + value.id + '">' + value.name + '</option>')
                        });
                    },
                    error: function (data) {
                    },
